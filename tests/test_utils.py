...
class TestInfoEmbed:
    ...
    def test_info_embed_creation(self):
        ...
        assert embed.color.value == 0x2B90D9, "デフォルトの色が設定されるべき"
    ...
    def test_info_embed_custom_color(self):
        ...
        assert embed.color.value == custom_color, "カスタム色が設定されるべき"
...
class TestMainMenuView:
    ...
    @pytest.mark.asyncio
    async def test_main_menu_view_creation(self):
        """メインメニューViewの作成をテスト"""
        view = main_menu_view()
    ...
    @pytest.mark.asyncio
    async def test_main_menu_view_buttons(self):
    ...
    @pytest.mark.asyncio
    async def test_main_menu_view_button_ids(self):
