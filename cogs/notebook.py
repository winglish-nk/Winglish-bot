...
class Notebook(commands.Cog):
        ...
        try:
            ...
            if not notebooks:
                await interaction.response.send_message(
                    ...
                    "または `/system_notebooks` でシステム推奨単語帳を確認できます。",
                    ephemeral=True
                )
                return
    ...
    @discord.app_commands.command(
        name="system_notebooks",
        description="システム推奨単語帳の一覧を表示"
    )
    async def system_notebooks(self, interaction: discord.Interaction) -> None:
        """システム推奨単語帳一覧を表示"""
        try:
            db_manager = get_db_manager()
            async with db_manager.acquire() as conn:
            ...
            await ErrorHandler.handle_interaction_error(
                interaction,
                e,
                log_context="notebook.system_notebooks"
            )
